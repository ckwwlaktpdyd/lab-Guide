...
### 제조자 콘솔 — `bench` 배경, 1133×744 고정
섹션 간격 24 · 카드 패딩 16 · **터치 타겟 최소 44pt**.

스플릿뷰 3단 실측 폭:

| 영역 | 폭 | 비고 |
|---|---|---|
| 다크 아이콘 레일 | ~88pt | 5항목 고정. 의뢰·편차에 미처리 건수 배지 |
| 리스트 | ~320pt | 상태 필터 칩 + 카드 목록 |
| 상세 패널 | ~730pt | 작업 공간 |

- 세로 감지 시 회전 안내 오버레이만 표시. **모바일 세로 레이아웃을 만들지 말 것**
- 완료된 공정 단계는 타임스탬프와 함께 처리, 현재 단계에만 입력 박스 인라인 노출
...
- [ ] 제조자 콘솔: 1133×744에서 스크롤 없이 한 화면, 터치 타겟 44pt 이상
